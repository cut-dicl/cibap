function [Cost pos PBQ_changed data]= temp(data,x,LoW, iteration,iii,PBQ_changed)
...
for i=1:length(AT) 
if BQ(i)==ABQ(i) && max(BQ)>1 % if proposed quay is ABQ, a fixed penalty is added
    pen=200;
...
HandlingCost=(HandlingTime-length(AT))*10; % we have assumed 10eur per hour handling cost and here unit is for 30min
